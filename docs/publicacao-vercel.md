...
- Domínio `creditoporperto.com` sob controle do proprietário.
...
4. **Domínios**: o domínio `creditoporperto.com` foi comprado dentro do
   próprio Vercel (registrar do time), então o DNS é gerenciado
   automaticamente. Adicionar `www.creditoporperto.com` como domínio
   principal do projeto e `creditoporperto.com` com redirect para o `www`
   (o next.config também força esse redirect);
...
6. **Deploy de produção**: merge/push na branch de produção configurada.
   Conferir: `https://www.creditoporperto.com/robots.txt`, `/sitemap.xml`,
...
- [ ] `curl -I https://creditoporperto.com/` → 308 para `https://www.…`
- [ ] `curl -I https://www.creditoporperto.com/` → 200, sem X-Robots-Tag
